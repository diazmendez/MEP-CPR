module param
  ...
  parameter (xlintol=1.d-4, nitmax = 10 * nnvar, ftol = 0.d0, gtol = 1.0d-5) !gtol-> 1.0d-7
  real*8 mutol    !tolerance of the projection of gradient into so
  !parameter (mutol=1.0d+300)!!!!!!!!!!!!!!!!!aqui no hace nada!!!!!!!!!!!!
...
600 format (2f20.8) !!!!!!!!!!!!!!!!!!!! this '2' must change as the dimension...
601 format (2f20.8,f20.8)!!!!!!!!!!!!!!!
  ...
  do k=1,actp
    !write(*,'(2e20.8)') mepst(:,k)
    write(*,601) mep(:,k), energy(mep(1,k),gr)
  enddo
  ...
  if (nsadd.ne.0) then 
     ...
     print *, '# There are not saddle points'
  endif
...
subroutine path_connection(pin,pfi)
  ...
  601 format (2f20.8,f20.8)!!!!!!!!!!!!!!! This 2 change as the dimension!
  ...
  if (mdp>2*pi/divi) then
     ...
     print *, '# ...distance is', mdp/(pi/divi),'times pi/divi' !adding info
     conn=.false.
  endif
...
print *, 'so final ->', so
     ...
     ! redefining mutol as in param: !!!!!!!!!!!!!!!!!!
     zmutol=0.5d0
     if ((abs(dot_product(XI,so)/sqrt(ggnew)))>zmutol) then
        print *, '# Gradient*So exceeded mu tolerance. in', ITS, 'iterations'
        print *, '#toler->',sqrt(real(N))*dot_product(XI,so)/sqrt(ggnew), zmutol
        return
     endif
